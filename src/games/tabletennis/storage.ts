// Table Tennis Storage Utilities

import { GameProgress } from './types';

const STORAGE_KEY = 'pingpong_pro_progress';

const DEFAULT_PROGRESS: GameProgress = {
  coins: 0,
  currentLevel: 1,
  highestLevel: 1,
  unlockedHeroes: ['wang', 'ding'],
  selectedHero: 'wang',
  completedLevels: [],
  bestScores: {},
  totalMatches: 0,
  ...
  difficulty: 'normal',
  completedMissions: [],
  heroUpgradeLevel: { 'wang': 1, 'ding': 1 },
};
...
export const storage = {
  get: (): GameProgress => {
    try {
      ...
      console.error('Failed to load ping pong progress:', e);
    }
  ...
  set: (progress: GameProgress): void => {
    try {
      ...
      console.error('Failed to save ping pong progress:', e);
    }
  },
  ...
  recordMatch: (won: boolean, spins: number): GameProgress => {
    const progress = storage.get();
    progress.totalMatches++;
    if (won) progress.totalWins++;
    progress.totalSpins += spins;
    storage.set(progress);
    return progress;
  },
