...
export const POWER_UP_CONFIGS: { [key in PowerUpType]: PowerUpConfig } = {
  slow_motion: {
    ...
    color: '#3498db',
    duration: 5,
    description: 'Time slows for easier returns',
    rarity: 0.2,
  },
  power_shot: {
    type: 'power_shot',
  ...
  auto_aim: {
    ...
    color: '#2ecc71',
    duration: 6,
    description: 'Ball targets corners',
    rarity: 0.15,
  },
  speed_boost: {
    type: 'speed_boost',
    ...
    color: '#f1c40f',
    duration: 7,
    description: 'Move faster side to side',
    rarity: 0.3,
  },
  super_spin: {
    type: 'super_spin',
    ...
    color: '#9b59b6',
    duration: 8,
    description: 'Insane spin on every shot',
    rarity: 0.1,
  },
};
...
export const createPowerUp = (tableWidth: number, tableHeight: number): PowerUp => {
  const type = getRandomPowerUpType();
  
  return {
    id: `powerup_${Date.now()}`,
    type,
    x: tableWidth * 0.2 + Math.random() * tableWidth * 0.6,
    y: tableHeight * 0.3 + Math.random() * tableHeight * 0.4,
    active: true,
